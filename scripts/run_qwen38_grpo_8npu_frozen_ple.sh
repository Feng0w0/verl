...
export VERL_FILE_LOGGER_ROOT="$PROJECT/artifacts/qwen38-grpo-8npu-frozen-ple/logs"
bash "$PROJECT/scripts/run_qwen38_grpo_4npu.sh" \
  ray_kwargs.ray_init.num_cpus=48 \
  ray_kwargs.ray_init._temp_dir=/tmp/ray-qwen38-grpo-8npu-ple \
  actor_rollout_ref.model.path=/mnt/weight/Qwen3.8-Flash-Next-3layer-frozen-ple \
  ...
  trainer.experiment_name=3layer_frozen_ple_grpo_8npu \
  trainer.default_local_dir="$PROJECT/artifacts/qwen38-grpo-8npu-frozen-ple/checkpoints" \
  trainer.rollout_data_dir="$PROJECT/artifacts/qwen38-grpo-8npu-frozen-ple/rollouts" \
  "$@"
